...
const validateTransaction = (transaction) => validator.validate(transaction, schemas.transactionSchema);
const validateTransactionType = (transactionType) => validator.validate(transactionType, schemas.userTransactionType);
const validateVehicle = (vehicle) => validator.validate(vehicle, schemas.vehicleSchema);
const validateParkingLot = (parkingLot) => validator.validate(parkingLot, schemas.parkingLotSchema);
const validateParkingHistory = (parkingHistory) => validator.validate(parkingHistory, schemas.parkingHistorySchema);
const validateVerifyState = (verifyState) => validator.validate(verifyState, schemas.verifyStateSchema);
const validateParkingPrice = (parkingPrice) => validator.validate(parkingPrice, schemas.parkingPriceSchema);
const validatePackage = (package) => validator.validate(package, schemas.packageSchema);
const validatePackageType = (packageType) => validator.validate(packageType, schemas.packageTypeSchema);
const validateUserPackage = (userPackage) => validator.validate(userPackage, schemas.userPackageSchema);
